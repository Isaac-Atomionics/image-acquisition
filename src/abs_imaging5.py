...
savepath = 'C:/Users/admin/Desktop/camera_stuff/images/Abs_imaging-2020-12-16/'
...
try:
    for i in range(num_run):
            ...
            # Start grabbing of images, unlimited amount, default type is continuous acquisition
                ...
                with cam.RetrieveResult(1000000) as result:
            
                    # Calling AttachGrabResultBuffer creates another reference to the
                    # grab result buffer. This prevents the buffer's reuse for grabbing.
                    img.AttachGrabResultBuffer(result)
                        
                    filename = savepath + folder_date + "-img_%05d_a.jpg" % (lastImageNum + i)
                    # Save image to
                ...
                with cam.RetrieveResult(2000) as result2:   
                    ...
                    filename = savepath + folder_date + "-img_%05d_b.jpg" % (lastImageNum + i)
                    # Save image to
                    img.Save(pylon.ImageFileFormat_Png, filename)
                    print(filename)
                    # In order to make it possible to reuse the grab result for grabbing
                ...
                with cam.RetrieveResult(2000) as result3:   
                    ...
                    filename = savepath + folder_date + "-img_%05d_c.jpg" % (lastImageNum + i)
                    # Save image to
                    img.Save(pylon.ImageFileFormat_Png, filename)
                    print(filename)
                    # In order to make it possible to reuse the grab result for grabbing
            ...
            #    with np.printoptions(threshold=np.inf):
               ...
               # plt.clim(0,1) # colourbar limit (,)
                ...
                cross_x = cross_x1[1024:1448]
                cross_z1 = OD[:,center_x]
                cross_z = cross_z1[100:800];
                ...
                ROI_sum=0
                for row in range(1124,1348):
                    for col in range(200,700):
               ...
               # print(filename)
                ...
                N_at = 2*pi*ODpk*sigma_x*sigma_z/sigma_0
                 
                  #%% Export parameters of interest
                 
                headline = ['Run #', 'filename','sigma_x','sigma_z','TOF','ROI_sum', 'I/Is','sigma_x fit error','sigma_z fit error']
                ...
                with open(savepath + folder_date + '-Data00000.csv', 'r') as file:
                    ...
                    rows = list(reader)
                    line_num = len(rows)
                parameters = [line_num,filename, sigma_x,sigma_z,tof, ROI_sum, IoverIs,err_x,err_z]
